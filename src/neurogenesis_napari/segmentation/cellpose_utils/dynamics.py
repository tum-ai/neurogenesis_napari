...
from scipy.ndimage import find_objects, maximum_filter1d
...
def _extend_centers_gpu(neighbors, meds, isneighbor, shape, n_iter=200, device=torch.device("cpu")):
    ...
    if T.ndim == 2:
        ...
        del grads
        mu_torch = np.stack(
            (dz.cpu().squeeze(0), dy.cpu().squeeze(0), dx.cpu().squeeze(0)),
            axis=-2,
        )
    return mu_torch
...
def steps_interp(dP, inds, niter, device=torch.device("cpu")):
    ...
    if (device.type == "cuda" or device.type == "mps") or ndim == 3:
        ...
        for n in range(ndim):
            if ndim == 3:
                pt[0, 0, 0, :, ndim - n - 1] = torch.from_numpy(inds[n]).to(
                    device, dtype=torch.float32
                )
            else:
                pt[0, 0, :, ndim - n - 1] = torch.from_numpy(inds[n]).to(
                    device, dtype=torch.float32
                )
...
def compute_masks(
    ...
    if (
    ...
    ).sum():  # mask at this point is a cell cluster binary map, not labels
        ...
        p_final = follow_flows(
            dP * (cellprob > cellprob_threshold) / 5.0,
            inds=inds,
            niter=niter,
            interp=interp,
            device=device,
