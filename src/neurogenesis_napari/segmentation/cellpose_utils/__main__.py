...
from cellpose_utils import denoise, io, models, train, utils, version_str
from cellpose_utils.cli import get_arg_parser

try:
    from cellpose_utils.gui import gui, gui3d
    ...
    if len(args.dir) == 0 and len(args.image_path) == 0:
        ...
        if not args.train and not args.train_size:
            ...
            if builtin_size and restore_type is None and not args.pretrained_model_ortho:
                ...
                if restore_type is None:
                    pretrained_model_ortho = (
                        None if args.pretrained_model_ortho is None else args.pretrained_model_ortho
                    )
            ...
            if args.diameter == 0:
                if builtin_size:
                    ...
                    if restore_type is None:
                        ...
                        logger.info(">>>> cannot auto-estimate diameter for image restoration")
            ...
            for image_name in tqdm(image_names, file=tqdm_out):
                ...
                if restore_type is not None:
                    imgs_dn = out[-1]
                    ratio = diams / model.dn.diam_mean if "upsample" in restore_type else 1.0
                    diams = (
                        model.dn.diam_mean
                        if "upsample" in restore_type and model.dn.diam_mean > diams
                        else diams
                    )
                else:
                    imgs_dn = None
            ...
            test_images, test_labels, image_names_test, test_probs = (
                None,
                None,
                None,
                None,
