...
from .core import assign_device, check_mkl, run_3D, run_net
...
def cache_model_path(basename):
    ...
    if not os.path.exists(cached_file):
        models_logger.info(f'Downloading: "{url}" to {cached_file}\n')
...
def get_model_params(pretrained_model, model_type, pretrained_model_ortho, default_model="cyto3"):
    ...
    if (
        pretrained_model
        and not Path(pretrained_model).exists()
        and np.any([pretrained_model == s for s in all_models])
    ):
...
class CellposeModel:
    ...
    def __init__(
        ...
        if self.pretrained_model:
            ...
            models_logger.info(">>>> no model weights loaded")
    ...
    def _run_net(
        ...
        if do_3D:
            ...
            if rescale != 1.0 or (anisotropy is not None and anisotropy != 1.0):
                models_logger.info(
                    f"resizing 3D image with rescale={rescale:.2f} and anisotropy={anisotropy}"
                )
    ...
    def _compute_masks(
        ...
        if do_3D:
            masks = dynamics.resize_and_compute_masks(
                ...
                max_size_fraction=max_size_fraction,
                resize=(
                    shape[:3] if (np.array(dP.shape[-3:]) != np.array(shape[:3])).sum() else None
                ),
                device=self.device,
            )
        else:
