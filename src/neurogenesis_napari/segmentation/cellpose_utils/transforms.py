...
def normalize99_tile(
    ...
    for z in range(Lz):
        IMG = np.zeros(
            (len(ystart), len(xstart), blocksizeY, blocksizeX, nchan),
            "float32",
        )
        k = 0
        for j in range(len(ystart)):
        ...
        for c in range(nchan):
            ...
            if to_fill.sum() > 0 and to_fill.sum() < x99_tiles[:, :, c].size:
                ...
                x01_tiles[fill_vals[0], fill_vals[1], c] = x01_tiles[
                    fill_neigh[0][nearest_neigh],
                    fill_neigh[1][nearest_neigh],
                    c,
                ]
...
def convert_image(x, channels, channel_axis=None, z_axis=None, do_3D=False, nchan=2):
    ...
    if x.ndim > 3:
        ...
        if len(to_squeeze) > 0:
            channel_axis = (
                update_axis(channel_axis, to_squeeze, x.ndim) if channel_axis is not None else None
            )
            z_axis = update_axis(z_axis, to_squeeze, x.ndim) if z_axis is not None else None
            x = x.squeeze()
        ...
        if random_per_image or n == 0:
            ...
            pts2 = np.float32(
                [
                    ...
                    cc1
                    + scale[n] * np.array([np.cos(np.pi / 2 + theta), np.sin(np.pi / 2 + theta)]),
                ]
            )
